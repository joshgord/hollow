...
public class HollowFactoryJavaGenerator extends HollowConsumerJavaFileGenerator {
    ...
    public String generate() {
        ...
        appendPackageAndCommonImports(builder, null, Collections.singletonList(schema));
        ...
        if(schema instanceof HollowListSchema)
            builder.append("import ").append(HollowListCachedDelegate.class.getName())
                .append(";\n");
        ...
        builder.append("public class ").append(className).append("<T extends ")
            .append(objectClassName).append("> extends HollowFactory<T> {\n\n");

        builder.append("    @Override\n");
        builder.append("    public T newHollowObject(HollowTypeDataAccess dataAccess, HollowTypeAPI typeAPI, int ordinal) {\n");
        builder.append("        return (T)new ").append(objectClassName).append("(((")
            .append(typeAPIClassname(schema.getName()))
            .append(")typeAPI).getDelegateLookupImpl(), ordinal);\n");
        builder.append("    }\n\n");

        builder.append("    @Override\n");
        builder.append("    public T newCachedHollowObject(HollowTypeDataAccess dataAccess, HollowTypeAPI typeAPI, int ordinal) {\n");
        builder.append("        return (T)new ").append(objectClassName).append("(new ")
            .append(delegateCachedClassname(schema)).append("((")
            .append(typeAPIClassname(schema.getName())).append(")typeAPI, ordinal), ordinal);\n");
        builder.append("    }\n\n");
